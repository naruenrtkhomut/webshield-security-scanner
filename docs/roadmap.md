# Roadmap

## Phase 1: MVP CLI

- [x] Repository structure
- [x] CLI skeleton
- [x] Core scanner orchestration
- [x] Security header checks
- [x] Cookie flag checks
- [x] Swagger/OpenAPI exposure check
- [x] Markdown report writer
- [x] Unit test project
- [x] GitHub Actions CI

## Phase 2: Developer-Friendly Reports

- [ ] Improve remediation messages for ASP.NET Core
- [ ] Add remediation messages for Angular hosting
- [ ] Add Nginx and Kubernetes examples
- [ ] Add HTML report export
- [ ] Add risk score summary
- [ ] Add executive summary section

## Phase 3: API and CI/CD Mode

- [ ] OpenAPI/Swagger schema parsing
- [ ] CI quality gate mode
- [ ] JSON output for automation
- [ ] Azure DevOps pipeline example
- [ ] GitHub Actions usage template

## Phase 4: Productization

- [ ] Landing page copy
- [ ] Gumroad product description
- [ ] Demo screenshots
- [ ] Sample vulnerable app for demo only
- [ ] Release packaging
- [ ] Versioned changelog

## Phase 5: SaaS Option

- [ ] Background scan scheduler
- [ ] Dashboard
- [ ] Multi-project support
- [ ] Team access
- [ ] Historical scan comparison
- [ ] Exportable PDF reports
